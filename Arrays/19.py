import random

arr1 = [3, 7, 1, 0, 4]
arr2 = [[2, 3], [7, 1], [0, 4]]

arr3 = [7 for i in range(5)]
arr4 = [i for i in range(1, 10)]
arr5 = [i * 2 for i in range(1, 10)]
arr6 = [random.randint(1, 20) for i in range(10)]

arr7 = [[] for i in range(5)]
arr8 = [[1 for i in range(2)] for j in range(4)]
arr9 = [[random.randint(1, 20) for i in range(3)] for j in range(5)]

arr10 = [4, 0, 3]
arr11 = [0] * 15
arr12 = [random.randint(1, 30) for i in range(10)]
arr13 = [random.randint(0, 1) for i in range(20)]
arr14 = [[False for i in range(2)] for j in range(5)]

print("--- 3.1 ARRAY CREATION ---")
print(f"arr1: {arr1}")
print(f"arr2: {arr2}")
print(f"arr3 (5x7): {arr3}")
print(f"arr4 (1..9): {arr4}")
print(f"arr5 (doubled): {arr5}")
print(f"arr6 (random): {arr6}")
print(f"arr7 (empty rows): {arr7}")
print(f"arr8 (4x2 ones): {arr8}")
print(f"arr9 (5x3 random): {arr9}")
print(f"arr10: {arr10}")
print(f"arr11 (15x0): {arr11}")
print(f"arr12 (random 1-30): {arr12}")
print(f"arr13 (random 0/1): {arr13}")
print(f"arr14 (5x2 False): {arr14}")
